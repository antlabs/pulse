...
mac: $(MAC_DIR)
	...
	GOOS=$(GOOS_MAC) $(GO_BUILD) $(GO_FLAGS) -o $(MAC_DIR)/client $(EXAMPLE_DIR)/client/client.go
	GOOS=$(GOOS_MAC) $(GO_BUILD) $(GO_FLAGS) -o $(MAC_DIR)/server $(EXAMPLE_DIR)/server/server.go
	@echo "macOS examples built successfully in $(MAC_DIR)/"

# Build Linux examples
linux: $(LINUX_DIR)
	@echo "Building Linux examples..."
	GOOS=$(GOOS_LINUX) $(GO_BUILD) $(GO_FLAGS) -o $(LINUX_DIR)/client $(EXAMPLE_DIR)/client/client.go
	GOOS=$(GOOS_LINUX) $(GO_BUILD) $(GO_FLAGS) -o $(LINUX_DIR)/server $(EXAMPLE_DIR)/server/server.go
